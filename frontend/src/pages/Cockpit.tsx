...
export default function Cockpit() {
  ...
  const runSummary = {
    ...
    awaiting: recentAgentRuns.filter((r) => r.status === "awaiting_approval")
      .length,
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={12} sm={6}>
          <Card>
            <Statistic
              title="活跃客户"
              value={keyMetrics.activeCustomers}
        ...
        <Col xs={12} sm={6}>
          <Card hoverable onClick={() => navigate("/workbench/approvals")}>
            <Statistic
              ...
              valueStyle={{
                color: pendingApprovals.length > 0 ? "#faad14" : undefined,
              }}
            />
          </Card>
        </Col>
            ...
            {riskSignals.length === 0 ? (
              ...
              <>
                <div style={{ marginBottom: 12 }}>
                  <Space size={16}>
                    <Tag color="red">
                      <WarningOutlined /> 高风险 {riskSummary.high}
                    </Tag>
                ...
                <List
                  ...
                  renderItem={(item) => (
                    <List.Item
                      ...
                      onClick={() =>
                        item.relatedId &&
                        navigate(`/customer-360/${item.relatedId}`)
                      }
                    >
                      <List.Item.Meta
                ...
                <div style={{ marginBottom: 12 }}>
                  <Space size={16}>
                    <Tag icon={<SyncOutlined spin />} color="processing">
                      运行中 {runSummary.running}
                    </Tag>
                    <Tag icon={<ClockCircleOutlined />} color="warning">
                      等待审批 {runSummary.awaiting}
                    </Tag>
                    <Tag icon={<CheckCircleOutlined />} color="success">
                      今日完成 {runSummary.succeeded}
                    </Tag>
                  </Space>
                </div>
                <List
                ...
                actions={[
                  todo.relatedType === "approval" ? (
                    ...
                    <Button
                      ...
                      size="small"
                      onClick={() =>
                        navigate(`/customer-360/${todo.relatedId}`)
                      }
                    >
                      查看
                    </Button>
