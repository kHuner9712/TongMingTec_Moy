...
import { Row, Col, Card, Typography, Badge, List, Tag, Space, Empty } from 'antd';
import {
  RobotOutlined,
  BellOutlined,
  MessageOutlined,
    ...
    <div>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={16}>
          ...
          >
            {aiInsights.length === 0 ? (
              <Empty description="暂无 AI 洞察" image={Empty.PRESENTED_IMAGE_SIMPLE} />
            ) : (
              <List
                ...
                renderItem={(item) => (
                  <List.Item>
                    ...
                    />
                    <Tag color={item.severity === 'warning' ? 'orange' : item.severity === 'error' ? 'red' : 'blue'}>
                      {item.type}
                    </Tag>
                  </List.Item>
                )}
          ...
          <Card title="快捷操作">
            <Space direction="vertical" style={{ width: '100%' }}>
              <a onClick={() => navigate('/customers')}>
                <Space><MessageOutlined /> 客户管理</Space>
              </a>
              <a onClick={() => navigate('/conversations')}>
                <Space><MessageOutlined /> 会话中心</Space>
              </a>
              <a onClick={() => navigate('/tickets')}>
                <Space><FileTextOutlined /> 工单管理</Space>
              </a>
              <a onClick={() => navigate('/agents')}>
                <Space><RobotOutlined /> Agent 管理</Space>
              </a>
            </Space>
          </Card>
        </Col>
