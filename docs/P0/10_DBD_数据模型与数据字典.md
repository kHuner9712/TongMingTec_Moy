...
| [05*BRD*业务需求说明书.md](./05_BRD_业务需求说明书.md)                    | v0.1 | 业务需求定义           |
| [06*PRD*产品需求规格说明书\_v0.1.md](./06_PRD_产品需求规格说明书_v0.1.md) | v0.1 | 字段定义、业务规则     |
| [07*RTM*需求跟踪矩阵.md](./07_RTM_需求跟踪矩阵.md)                        | v0.1 | 需求跟踪               |
| [09*HLD*系统高层设计.md](./09_HLD_系统高层设计.md)                        | v0.1 | 业务对象定义、对象关系 |
...
| departments      | 部门表         | 存储组织部门结构     | organizations      | P0     |
...
| 表名  | 中文名 | 作用         | 关键关联                        | 优先级 |
| ----- | ------ | ------------ | ------------------------------- | ------ |
| tasks | 任务表 | 存储待办任务 | organizations, users, customers | P0     |

### 6.10 通知中心模块

| 表名          | 中文名 | 作用         | 关键关联             | 优先级 |
| ------------- | ------ | ------------ | -------------------- | ------ |
| notifications | 通知表 | 存储系统通知 | organizations, users | P0     |

### 6.11 渠道管理模块

| 表名     | 中文名 | 作用             | 关键关联      | 优先级 |
| -------- | ------ | ---------------- | ------------- | ------ |
| channels | 渠道表 | 存储客户来源渠道 | organizations | P0     |

### 6.12 自动化规则模块

| 表名             | 中文名       | 作用               | 关键关联      | 优先级 |
| ---------------- | ------------ | ------------------ | ------------- | ------ |
| automation_rules | 自动化规则表 | 存储自动化规则配置 | organizations | P1     |

### 6.13 AI工作台模块

| 表名     | 中文名   | 作用               | 关键关联      | 优先级 |
| -------- | -------- | ------------------ | ------------- | ------ |
| ai_tasks | AI任务表 | 存储AI任务执行记录 | organizations | P1     |
...
| permissions           | 权限表       | 存储权限定义     | -            | P0     |
| menus                 | 菜单表       | 存储菜单定义     | -            | P0     |
| role_menus            | 角色菜单关联 | 角色与菜单关联   | roles, menus | P0     |
| field_permissions     | 字段权限表   | 字段级权限配置   | roles        | P1     |
| data_scopes           | 数据范围表   | 数据范围权限配置 | roles        | P1     |
| temporary_permissions | 临时权限表   | 临时授权记录     | users        | P1     |

### 6.16 审计日志模块

| 表名             | 中文名         | 作用           | 关键关联         | 优先级 |
| ---------------- | -------------- | -------------- | ---------------- | ------ |
| login_logs       | 登录日志表     | 登录记录       | users            | P0     |
| data_change_logs | 数据变更日志表 | 数据变更记录   | users            | P0     |
| ai_task_logs     | AI任务日志表   | AI执行记录     | ai_tasks         | P1     |
| automation_logs  | 自动化日志表   | 自动化执行记录 | automation_rules | P1     |
| integration_logs | 集成日志表     | 外部调用记录   | integrations     | P1     |

### 6.17 配置中心模块

| 表名                | 中文名       | 作用           | 关键关联      | 优先级 |
| ------------------- | ------------ | -------------- | ------------- | ------ |
| dict_types          | 字典类型表   | 字典类型定义   | -             | P0     |
| dict_items          | 字典项表     | 字典项定义     | dict_types    | P0     |
| system_configs      | 系统配置表   | 系统级配置     | -             | P0     |
| org_configs         | 租户配置表   | 租户级配置     | organizations | P0     |
| status_configs      | 状态配置表   | 状态枚举配置   | -             | P1     |
| sla_rules           | SLA规则表    | SLA规则配置    | -             | P1     |
| ticket_categories   | 工单分类表   | 工单分类配置   | sla_rules     | P0     |
| ai_prompt_templates | AI提示词模板 | AI提示词模板   | -             | P1     |
| ai_model_configs    | AI模型配置表 | AI模型参数配置 | -             | P1     |
| sensitive_words     | 敏感词表     | 敏感词配置     | -             | P1     |

### 6.18 集成管理模块

| 表名            | 中文名        | 作用             | 关键关联 | 优先级 |
| --------------- | ------------- | ---------------- | -------- | ------ |
| integrations    | 集成配置表    | 外部系统集成配置 | -        | P0     |
| webhooks        | Webhook配置表 | Webhook配置      | -        | P0     |
| webhook_events  | Webhook事件表 | Webhook事件记录  | webhooks | P1     |
| channel_configs | 渠道配置表    | IM渠道配置       | -        | P0     |
| api_keys        | API Key表     | API密钥管理      | users    | P0     |
| oauth_configs   | OAuth配置表   | OAuth集成配置    | -        | P1     |
| sso_configs     | SSO配置表     | SSO单点登录配置  | -        | P1     |
...
#### 7.1.2 departments（部门表）

| 字段名     | 中文说明   | 类型        | 必填 | 默认值            | 枚举值/示例      | 索引    | 备注                  |
| ---------- | ---------- | ----------- | ---- | ----------------- | ---------------- | ------- | --------------------- |
| id         | 部门ID     | BIGSERIAL   | 是   | 自增              | -                | PK      | 主键                  |
| org_id     | 组织ID     | BIGINT      | 是   | -                 | -                | FK, IDX | 外键关联organizations |
| parent_id  | 父部门ID   | BIGINT      | 否   | -                 | -                | FK, IDX | NULL表示顶级部门      |
| name       | 部门名称   | VARCHAR(64) | 是   | -                 | 销售部           | -       |                       |
| code       | 部门编码   | VARCHAR(32) | 否   | -                 | SALES            | UK      |                       |
| manager_id | 部门负责人 | BIGINT      | 否   | -                 | -                | FK, IDX | 关联users             |
| sort_order | 排序       | INTEGER     | 是   | 0                 | -                | -       |                       |
| status     | 状态       | VARCHAR(16) | 是   | active            | active, inactive | IDX     |                       |
| created_at | 创建时间   | TIMESTAMP   | 是   | CURRENT_TIMESTAMP | -                | -       |                       |
| updated_at | 更新时间   | TIMESTAMP   | 是   | CURRENT_TIMESTAMP | -                | -       | 触发器更新            |
...
- PRIMARY KEY: id
- UNIQUE KEY: uk_departments_code (org_id, code, is_deleted)
- FOREIGN KEY: fk_departments_org (org_id) REFERENCES organizations(id)
- FOREIGN KEY: fk_departments_parent (parent_id) REFERENCES departments(id)
- FOREIGN KEY: fk_departments_manager (manager_id) REFERENCES users(id)
...
| status     | 生命周期状态 | VARCHAR(16)  | 是   | potential         | potential, new_customer, active, silent, lost | IDX     | 见客户生命周期状态机 |
...
| 字段名       | 中文说明     | 类型         | 必填 | 默认值            | 枚举值/示例                                | 索引    | 备注 |
| ------------ | ------------ | ------------ | ---- | ----------------- | ------------------------------------------ | ------- | ---- |
| id           | 任务ID       | BIGSERIAL    | 是   | 自增              | -                                          | PK      | 主键 |
| org_id       | 组织ID       | BIGINT       | 是   | -                 | -                                          | FK, IDX |      |
| title        | 任务标题     | VARCHAR(200) | 是   | -                 | 跟进客户张三                               | IDX     |      |
| description  | 任务描述     | TEXT         | 否   | -                 | -                                          | -       |      |
| assignee_id  | 负责人ID     | BIGINT       | 是   | -                 | -                                          | FK, IDX |      |
| creator_id   | 创建人ID     | BIGINT       | 是   | -                 | -                                          | FK, IDX |      |
| related_type | 关联对象类型 | VARCHAR(32)  | 否   | -                 | customer, lead, opportunity, ticket        | IDX     |      |
| related_id   | 关联对象ID   | BIGINT       | 否   | -                 | -                                          | IDX     |      |
| priority     | 优先级       | VARCHAR(16)  | 是   | medium            | high, medium, low                          | IDX     |      |
| status       | 状态         | VARCHAR(16)  | 是   | pending           | pending, in_progress, completed, cancelled | IDX     |      |
| due_at       | 截止时间     | TIMESTAMP    | 否   | -                 | -                                          | IDX     |      |
| remind_at    | 提醒时间     | TIMESTAMP    | 否   | -                 | -                                          | -       |      |
| completed_at | 完成时间     | TIMESTAMP    | 否   | -                 | -                                          | -       |      |
| created_at   | 创建时间     | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                                          | IDX     |      |
| updated_at   | 更新时间     | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                                          | -       |      |
| created_by   | 创建人       | BIGINT       | 是   | -                 | -                                          | FK      |      |
| updated_by   | 更新人       | BIGINT       | 是   | -                 | -                                          | FK      |      |
...
- PRIMARY KEY: id
- FOREIGN KEY: fk_tasks_org (org_id) REFERENCES organizations(id)
- FOREIGN KEY: fk_tasks_assignee (assignee_id) REFERENCES users(id)
- FOREIGN KEY: fk_tasks_creator (creator_id) REFERENCES users(id)

---

### 7.10 通知中心模块

#### 7.10.1 notifications（通知表）

| 字段名       | 中文说明     | 类型         | 必填 | 默认值            | 枚举值/示例                                     | 索引    | 备注 |
| ------------ | ------------ | ------------ | ---- | ----------------- | ----------------------------------------------- | ------- | ---- |
| id           | 通知ID       | BIGSERIAL    | 是   | 自增              | -                                               | PK      | 主键 |
| org_id       | 组织ID       | BIGINT       | 是   | -                 | -                                               | FK, IDX |      |
| user_id      | 接收用户ID   | BIGINT       | 是   | -                 | -                                               | FK, IDX |      |
| type         | 通知类型     | VARCHAR(32)  | 是   | -                 | lead_assign, task_remind, ticket_assign, system | IDX     |      |
| title        | 通知标题     | VARCHAR(128) | 是   | -                 | 新线索分配                                      | -       |      |
| content      | 通知内容     | TEXT         | 是   | -                 | -                                               | -       |      |
| related_type | 关联对象类型 | VARCHAR(32)  | 否   | -                 | lead, task, ticket, conversation                | IDX     |      |
| related_id   | 关联对象ID   | BIGINT       | 否   | -                 | -                                               | IDX     |      |
| is_read      | 已读标识     | SMALLINT     | 是   | 0                 | 0, 1                                            | IDX     |      |
...
- PRIMARY KEY: id
- FOREIGN KEY: fk_notifications_org (org_id) REFERENCES organizations(id)
- FOREIGN KEY: fk_notifications_user (user_id) REFERENCES users(id)
...
| 字段名       | 中文说明 | 类型         | 必填 | 默认值            | 枚举值/示例               | 索引    | 备注 |
| ------------ | -------- | ------------ | ---- | ----------------- | ------------------------- | ------- | ---- |
| id           | 渠道ID   | BIGSERIAL    | 是   | 自增              | -                         | PK      | 主键 |
| org_id       | 组织ID   | BIGINT       | 是   | -                 | -                         | FK, IDX |      |
| name         | 渠道名称 | VARCHAR(64)  | 是   | -                 | 官网表单, 微信公众号      | -       |      |
| code         | 渠道编码 | VARCHAR(32)  | 是   | -                 | website, wechat           | UK      |      |
| channel_type | 渠道类型 | VARCHAR(16)  | 是   | -                 | online, offline, referral | IDX     |      |
| description  | 渠道描述 | VARCHAR(256) | 否   | -                 | -                         | -       |      |
| status       | 状态     | VARCHAR(16)  | 是   | active            | active, inactive          | IDX     |      |
| lead_count   | 线索数量 | INTEGER      | 是   | 0                 | -                         | -       | 统计 |
| created_at   | 创建时间 | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                         | -       |      |
| updated_at   | 更新时间 | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                         | -       |      |
| created_by   | 创建人   | BIGINT       | 是   | -                 | -                         | FK      |      |
| updated_by   | 更新人   | BIGINT       | 是   | -                 | -                         | FK      |      |
...
- PRIMARY KEY: id
- UNIQUE KEY: uk_channels_code (org_id, code, is_deleted)
- FOREIGN KEY: fk_channels_org (org_id) REFERENCES organizations(id)

**预置渠道：**

| code     | name       | channel_type | 说明           |
| -------- | ---------- | ------------ | -------------- |
| website  | 官网表单   | online       | 官网咨询表单   |
| wechat   | 微信公众号 | online       | 微信公众号咨询 |
...
#### 7.12.1 automation_rules（自动化规则表）

| 字段名        | 中文说明 | 类型         | 必填 | 默认值            | 枚举值/示例                                                                        | 索引    | 备注     |
| ------------- | -------- | ------------ | ---- | ----------------- | ---------------------------------------------------------------------------------- | ------- | -------- |
| id            | 规则ID   | BIGSERIAL    | 是   | 自增              | -                                                                                  | PK      | 主键     |
| org_id        | 组织ID   | BIGINT       | 是   | -                 | -                                                                                  | FK, IDX |          |
| name          | 规则名称 | VARCHAR(64)  | 是   | -                 | 线索自动分配规则                                                                   | -       |          |
| code          | 规则编码 | VARCHAR(32)  | 是   | -                 | lead_assign                                                                        | UK      |          |
| rule_type     | 规则类型 | VARCHAR(16)  | 是   | -                 | lead_assign, conversation_assign, ticket_assign, notification, escalation, recycle | IDX     |          |
| trigger_event | 触发事件 | VARCHAR(32)  | 是   | -                 | lead_create, ticket_create                                                         | -       |          |
| conditions    | 触发条件 | JSONB        | 否   | -                 | {"source": "website"}                                                              | -       |          |
| actions       | 执行动作 | JSONB        | 是   | -                 | {"type": "assign", "method": "round_robin"}                                        | -       |          |
| priority      | 优先级   | INTEGER      | 是   | 0                 | -                                                                                  | -       | 执行顺序 |
| status        | 状态     | VARCHAR(16)  | 是   | active            | active, inactive                                                                   | IDX     |          |
| description   | 规则描述 | VARCHAR(256) | 否   | -                 | -                                                                                  | -       |          |
| created_at    | 创建时间 | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                                                                                  | -       |          |
| updated_at    | 更新时间 | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                                                                                  | -       |          |
| created_by    | 创建人   | BIGINT       | 是   | -                 | -                                                                                  | FK      |          |
| updated_by    | 更新人   | BIGINT       | 是   | -                 | -                                                                                  | FK      |          |
...
- PRIMARY KEY: id
- UNIQUE KEY: uk_automation_rules_code (org_id, code, is_deleted)
- FOREIGN KEY: fk_automation_rules_org (org_id) REFERENCES organizations(id)

---

### 7.13 AI工作台模块

#### 7.13.1 ai_tasks（AI任务表）

| 字段名        | 中文说明     | 类型         | 必填 | 默认值            | 枚举值/示例                           | 索引    | 备注 |
| ------------- | ------------ | ------------ | ---- | ----------------- | ------------------------------------- | ------- | ---- |
| id            | 任务ID       | BIGSERIAL    | 是   | 自增              | -                                     | PK      | 主键 |
| org_id        | 组织ID       | BIGINT       | 是   | -                 | -                                     | FK, IDX |      |
| task_type     | 任务类型     | VARCHAR(32)  | 是   | -                 | smart_reply, knowledge_qa, lead_score | IDX     |      |
| related_type  | 关联对象类型 | VARCHAR(32)  | 否   | -                 | conversation, lead, knowledge         | IDX     |      |
| related_id    | 关联对象ID   | BIGINT       | 否   | -                 | -                                     | IDX     |      |
| input_data    | 输入数据     | JSONB        | 是   | -                 | -                                     | -       |      |
| output_data   | 输出数据     | JSONB        | 否   | -                 | -                                     | -       |      |
| status        | 状态         | VARCHAR(16)  | 是   | pending           | pending, running, completed, failed   | IDX     |      |
| error_message | 错误信息     | VARCHAR(512) | 否   | -                 | -                                     | -       |      |
| confidence    | 置信度       | DECIMAL(5,4) | 否   | -                 | 0.8523                                | -       |      |
| model_name    | 模型名称     | VARCHAR(64)  | 否   | -                 | gpt-4, gpt-3.5-turbo                  | -       |      |
| tokens_used   | Token消耗    | INTEGER      | 否   | -                 | -                                     | -       |      |
| duration_ms   | 耗时(毫秒)   | INTEGER      | 否   | -                 | -                                     | -       |      |
| started_at    | 开始时间     | TIMESTAMP    | 否   | -                 | -                                     | -       |      |
| completed_at  | 完成时间     | TIMESTAMP    | 否   | -                 | -                                     | -       |      |
| created_at    | 创建时间     | TIMESTAMP    | 是   | CURRENT_TIMESTAMP | -                                     | IDX     |      |
...
- PRIMARY KEY: id
- FOREIGN KEY: fk_ai_tasks_org (org_id) REFERENCES organizations(id)
...
| 状态值       | 中文名   | 说明                 |
| ------------ | -------- | -------------------- |
| potential    | 潜在客户 | 尚未建立正式业务关系 |
| new_customer | 新客户   | 刚建立业务关系       |
| active       | 活跃客户 | 正在合作中           |
| silent       | 沉默客户 | 长期无互动           |
| lost         | 流失客户 | 已流失               |
...
| --------- | ------ | -------------------- |
| draft     | 草稿   | 草稿状态，未发布     |
| published | 已发布 | 已发布状态，可被检索 |
| archived  | 已归档 | 已归档状态，不可检索 |
...
| 客户生命周期 | 潜在/新客户/活跃/沉默/流失                    | potential/new_customer/active/silent/lost         | ✅ 一致  |
| 线索手机号   | 必填                                          | 必填                                              | ✅ 一致  |
| 线索状态     | 新线索/已分配/跟进中/已转化/已失效            | new/assigned/following/converted/invalid          | ✅ 一致  |
...
| 工单状态     | 待处理/已分配/处理中/已解决/已关闭            | pending/assigned/processing/resolved/closed       | ✅ 一致  |
| 任务状态     | 待处理/进行中/已完成/已取消                   | pending/in_progress/completed/cancelled           | ✅ 一致  |
| AI任务状态   | 待执行/执行中/已完成/失败                     | pending/running/completed/failed                  | ✅ 一致  |

### 14.2 状态机对齐说明

| 对象             | PRD章节 | DBD章节 | 对齐状态    |
| ---------------- | ------- | ------- | ----------- |
| 线索状态机       | 4.5.4   | 8.1     | ✅ 完全对齐 |
| 商机阶段流转     | 4.7.4   | 8.2     | ✅ 完全对齐 |
| 工单状态机       | 4.10.4  | 8.3     | ✅ 完全对齐 |
| 会话状态机       | 4.8.4   | 8.4     | ✅ 完全对齐 |
| 客户生命周期     | 4.4.4   | 8.5     | ✅ 完全对齐 |
| 任务状态机       | 4.9.4   | 8.6     | ✅ 完全对齐 |
| AI任务状态机     | 4.13.3  | 8.7     | ✅ 完全对齐 |
| 用户状态机       | 6.3.1   | 8.8     | ✅ 完全对齐 |
| 组织状态机       | 6.3.2   | 8.9     | ✅ 完全对齐 |
| 部门状态机       | 6.3.3   | 8.10    | ✅ 完全对齐 |
| 自动化规则状态机 | 6.3.4   | 8.11    | ✅ 完全对齐 |
| 知识库条目状态机 | 6.3.5   | 8.12    | ✅ 完全对齐 |
...
| v1.0 | 2026-04-04 | MOY 文档架构组 | 更新与PRD一致性检查：状态机定义已完全对齐                                                                                                                     | 正式版 |
| v2.0 | 2026-04-05 | MOY 文档架构组 | 全量对账补完：新增departments/tasks/notifications/channels/automation_rules/ai_tasks表；新增客户生命周期/任务/AI任务状态机；修正customers.status字段与PRD一致 | 已确认 |
| v3.0 | 2026-04-05 | MOY 文档架构组 | 企业级横向能力补完：新增权限管理/审计日志/配置中心/集成管理模块表定义；总表数从31增至50                                                                       | 已确认 |
| v3.1 | 2026-04-05 | MOY 文档架构组 | 一致性治理：补充用户/组织/部门/自动化规则/知识库条目共5个状态机定义，状态机总数从7个增至12个                                                                  | 已确认 |
