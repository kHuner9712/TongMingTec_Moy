...
export class PromptTemplateService {
  ...
  async create(orgId: string, dto: CreatePromptTemplateDto): Promise<AiPromptTemplate> {
  ...
  async update(id: string, orgId: string, data: Partial<AiPromptTemplate>): Promise<AiPromptTemplate> {
  ...
  async getTemplateByCode(templateCode: string, agentCode: string, orgId: string): Promise<AiPromptTemplate | null> {
  ...
  async listTemplates(orgId: string, filters?: { agentCode?: string }): Promise<AiPromptTemplate[]> {
    const qb = this.templateRepo.createQueryBuilder('tpl').where('tpl.orgId = :orgId', { orgId });
    if (filters?.agentCode) qb.andWhere('tpl.agentCode = :agentCode', { agentCode: filters.agentCode });
    qb.orderBy('tpl.agentCode', 'ASC').addOrderBy('tpl.version', 'DESC');
    return qb.getMany();
  }
}
