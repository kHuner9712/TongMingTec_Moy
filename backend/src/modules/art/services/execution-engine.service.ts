...
export class ExecutionEngineService {
  ...
  async execute(
    ...
    try {
      const output = await this.executeByMode(agent, input, savedRun.id, orgId, userId, customerId);
      ...
      const updatedRun = await this.runRepo.findOne({ where: { id: savedRun.id } });
      ...
      const failedRun = await this.runRepo.findOne({ where: { id: savedRun.id } });
  ...
  private async executeByMode(
    ...
    userId: string,
    customerId?: string,
  ): Promise<{ status: AgentRunStatus; outputPayload: Record<string, unknown> }> {
    switch (agent.executionMode) {
      ...
      case 'approval':
        return this.executeApproval(agent, input, runId, orgId, userId, customerId);
      default:
        return this.executeSuggest(agent, input);
    }
  }
  ...
  private async executeAuto(
    ...
    if (agent.riskLevel !== 'low') {
      return {
        status: AgentRunStatus.FAILED,
        outputPayload: { error: 'AUTO_MODE_ONLY_ALLOWED_FOR_LOW_RISK', mode: 'auto' },
      };
    }
  ...
  private async executeApproval(
    ...
    await this.approvalEngine.createApprovalRequest(runId, orgId, {
      ...
      proposedAfterSnapshot: (input.proposedAfterSnapshot as Record<string, unknown>) || null,
  ...
  private generateSuggestions(agent: AiAgent, input: Record<string, unknown>): Array<{ id: string; content: string; confidence: number }> {
    if (agent.code === 'AGENT-AI-003') {
      return [
        { id: '1', content: '感谢您的咨询，我来为您详细解答。', confidence: 0.92 },
        { id: '2', content: '您好，请问有什么可以帮助您的？', confidence: 0.88 },
        { id: '3', content: '好的，我马上为您处理这个问题。', confidence: 0.85 },
      ];
    }
    return [
      { id: '1', content: `Agent ${agent.name} 建议执行 ${agent.agentType} 操作`, confidence: 0.80 },
    ];
  }

  private generateDraft(agent: AiAgent, input: Record<string, unknown>): Record<string, unknown> {
    return {
      agentCode: agent.code,
  ...
  private generateAutoResult(agent: AiAgent, input: Record<string, unknown>): Record<string, unknown> {
    return {
      agentCode: agent.code,
  ...
  async listRuns(orgId: string, filters?: { agentId?: string; status?: string; customerId?: string }): Promise<AiAgentRun[]> {
    const qb = this.runRepo.createQueryBuilder('run').where('run.orgId = :orgId', { orgId });
    if (filters?.agentId) qb.andWhere('run.agentId = :agentId', { agentId: filters.agentId });
    if (filters?.status) qb.andWhere('run.status = :status', { status: filters.status });
    if (filters?.customerId) qb.andWhere('run.customerId = :customerId', { customerId: filters.customerId });
