...
import { fetchReports, GeoReportBrief, QueryReportsParams } from "./reports/geoReportsApi";
import { C, sans } from "../styles";

const STATUS_LABELS: Record<string, string> = {
  draft: "草稿", ready: "就绪", delivered: "已交付", archived: "已归档",
};
const STATUS_COLORS: Record<string, string> = {
  draft: "#8a9aaa", ready: "#0055cc", delivered: "#0f7b3a", archived: "#a0a0a0",
};
const ALL_STATUSES = ["", "draft", "ready", "delivered", "archived"];

export default function ReportsListPage() {
  const [data, setData] = useState<GeoReportBrief[]>([]);
  ...
  const load = (p: number) => {
    setLoading(true);
    const params: QueryReportsParams = { page: p, pageSize };
    ...
    <div style={{ fontFamily: sans, color: C.dark, background: C.bg, minHeight: "100vh" }}>
      <nav style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <a href="/admin/reports/new" style={{ fontSize: 13, color: "#fff", textDecoration: "none", fontWeight: 600, background: "#0055cc", padding: "6px 14px", borderRadius: 6 }}>新建报告</a>
          <a href="/" style={{ fontSize: 13, color: C.blue, textDecoration: "none", fontWeight: 600 }}>← 返回官网</a>
        </div>
      </nav>

      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 20 }}>诊断报告列表</h1>
        ...
        {!loading && data.length === 0 && (
          <div style={{ textAlign: "center", padding: 40, color: "#8a9aaa", border: "1px dashed #d0d7de", borderRadius: 10 }}>
            暂无报告，<a href="/admin/reports/new" style={{ color: C.blue }}>新建报告</a>
          </div>
        )}
            ...
            <tbody>
              {data.map((r) => (
                <tr key={r.id} style={{ borderBottom: "1px solid #f0f2f5" }}>
                  ...
                  <td style={td}><a href={r.website || "#"} target="_blank" rel="noopener" style={{ color: C.blue, fontSize: 13 }}>{r.website}</a></td>
                  <td style={td}>{r.industry || "-"}</td>
                  <td style={td}>
                    <span style={{ padding: "2px 10px", borderRadius: 10, fontSize: 12, fontWeight: 600, background: STATUS_COLORS[r.status] + "18", color: STATUS_COLORS[r.status] }}>
                      {STATUS_LABELS[r.status] || r.status}
                    </span>
                  </td>
                  <td style={td}>{new Date(r.createdAt).toLocaleDateString("zh-CN")}</td>
                  <td style={td}>
                    <a href={`/admin/reports/new?reportId=${r.id}`} style={{ color: C.blue, fontSize: 13, fontWeight: 600, textDecoration: "none" }}>编辑</a>
                  </td>
                </tr>
              ))}
            </tbody>
